...
public class GameCanvasImplementation extends LWUITImplementation {
    ...
    private int[] rgbArr;
    private int rgbArrFilledLength = 0;     // BlueWhaleSystems fix - ticket:3536
     ...
     */
    static int[] rightSK = new int[]{-7, -7};    // BlueWhaleSystems fix - ticket:4005
    /**
    ...
    public void init(Object m) {
         ...
         *
         * - ticket:3482 "LWUIT: check flushGraphicsBug with our VM"
         */
       if (platform != null && platform.toUpperCase().indexOf("BLUEWHALE") >= 0) {
           flushGraphicsBug = false;
           Display.getInstance().setTransitionYield(-1);
       } else if (platform != null && platform.toUpperCase().indexOf("NOKIA") >= 0) {
            flushGraphicsBug = false;
            NOKIA = true;
...
// Check Third Soft Button is supported        
     ...
     * 
     * BlueWhaleSystems fix -- Michael Maguire -- 06 Jul 2009
     * 
     * Scope changed to public because we need to be able to get the implementation
     * to as for the underlying LWUIT canvas to understand when we're being 
     * foregrounded.
     * 
     * See ticket:3205 LWUIT: Foregrounding on new item arrival no longer working
     *
    ...
    public void fillRect(Object graphics, int x, int y, int w, int h) {
        ...
        if (alpha == 0xff) {
            ...
            /** 
              * BlueWhaleSystems fix: Darren Jones
              *
              * - ticket:3536 "LWUIT: Optimise GameCanvasImplementation.fillRect()"
              */
            if (rgbArr != null && rgbArr.length >= w) {
                if (rgbArr[0] != color) {
                    rgbArrFilledLength = 0;
                }
            }
            else {
                rgbArr = new int[w];
            ...
            for (int i = rgbArrFilledLength; i <
                    w; i++) {
                rgbArr[i] = color;
            }
            rgbArrFilledLength = Math.max(rgbArrFilledLength, w);
            ...
            /** 
              ...
              *
              * - ticket:3536 "LWUIT: Optimise GameCanvasImplementation.fillRect()"
              * NOTE: On 11/3/10, MJB merged LWUIT's latest source into Darren's optimization.
              */
            if (rgbX >= 0) {
                javax.microedition.lcdui.Graphics nativeGraphics = (javax.microedition.lcdui.Graphics) graphics;
                for (int i = Math.max(0, -rgbY); i < h; i++) {
                    int currentY = rgbY + i;
                    if(currentY >= clipY && currentY <= clipBottomY) {
                        nativeGraphics.drawRGB(rgbArr, 0, w, rgbX, currentY, w, 1, true);
                    }
                }
            }
	...
	public void editString(Component cmp, int maxSize, int constraint,
