...
import com.duchuyctlk.moneytracker.model.ThuChiModel;
 ...
 * @author duc_huy_ctlk
 */
public class ThuChiController implements IController{

    private ThuChiModel model;
    private DatabaseService db;

    public ThuChiController(IModel value) {
        model = (ThuChiModel) value;
        db = new DatabaseService("thuchi");
    }

    public void loadMoneyAmounts() {
        db.open();
        ArrayList entries = db.readRecords(new String[]{"", "", "", "", "", ""});
    ...
    public void loadMoneyAmounts(int soLuong, int loai, String noiDung, int ng, int th, int nm ) {
        db.open();
        String[] params = new String[]{"", "", "", "", "", ""};

        if (soLuong > 0)
            params[0] = Integer.toString(soLuong);
        if (loai > -1)
            params[1] = Integer.toString(loai);
        if (!noiDung.equals(""))
            params[2] = noiDung;
        if (ng > 0)
            params[3] = Integer.toString(ng);
        if (th > 0)
            params[4] = Integer.toString(th);
        if (nm > 0)
            params[5] = Integer.toString(nm);
        
        ArrayList entries = db.readRecords(params);
